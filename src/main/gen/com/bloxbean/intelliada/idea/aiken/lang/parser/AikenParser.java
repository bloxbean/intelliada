...
public class AikenParser implements PsiParser, LightPsiParser {
  ...
  /* ********************************************************** */
  // topLevelDefinition*
  static boolean gleamFile(PsiBuilder b, int l) {
    if (!recursion_guard_(b, l, "gleamFile")) return false;
    while (true) {
      int c = current_position_(b);
      if (!topLevelDefinition(b, l + 1)) break;
      if (!empty_element_parsed_guard_(b, "gleamFile", c)) break;
    }
    return true;
  }
  ...
  static boolean topLevelDefinition(PsiBuilder b, int l) {
    ...
    if (!r) r = topLevelDefinition_4(b, l + 1);
    exit_section_(b, m, null, r);
    return r;
  }

  // [PUB] constantStatement
  private static boolean topLevelDefinition_4(PsiBuilder b, int l) {
    if (!recursion_guard_(b, l, "topLevelDefinition_4")) return false;
    boolean r;
    Marker m = enter_section_(b);
    r = topLevelDefinition_4_0(b, l + 1);
  ...
  // [PUB]
  private static boolean topLevelDefinition_4_0(PsiBuilder b, int l) {
    if (!recursion_guard_(b, l, "topLevelDefinition_4_0")) return false;
    consumeToken(b, PUB);
    return true;
  }
  ...
  /* ********************************************************** */
  // [PUB] TYPE UPPER_IDENTIFIER LBRACE (typeStatementConstructor)* RBRACE
  public static boolean typeStatement(PsiBuilder b, int l) {
    ...
    r = r && consumeTokens(b, 0, TYPE, UPPER_IDENTIFIER, LBRACE);
    r = r && typeStatement_4(b, l + 1);
    r = r && consumeToken(b, RBRACE);
    exit_section_(b, l, m, r, false, null);
    return r;
  }
  ...
  private static boolean typeStatement_4(PsiBuilder b, int l) {
    if (!recursion_guard_(b, l, "typeStatement_4")) return false;
    while (true) {
      int c = current_position_(b);
      if (!typeStatement_4_0(b, l + 1)) break;
      if (!empty_element_parsed_guard_(b, "typeStatement_4", c)) break;
    }
    return true;
  }

  // (typeStatementConstructor)
  private static boolean typeStatement_4_0(PsiBuilder b, int l) {
    if (!recursion_guard_(b, l, "typeStatement_4_0")) return false;
    boolean r;
    Marker m = enter_section_(b);
  ...
  /* ********************************************************** */
  // UPPER_IDENTIFIER LPAREN (constructorElement)* RPAREN
  public static boolean typeStatementConstructor(PsiBuilder b, int l) {
    ...
    boolean r;
    Marker m = enter_section_(b);
    r = consumeTokens(b, 0, UPPER_IDENTIFIER, LPAREN);
